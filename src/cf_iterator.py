...
# collections.abc: 容器的抽象基类	collections.abc.Iterable: 提供了 __iter__() 方法的抽象基类
...
class CFIterator:
	def __init__(
		...
		self._arch = arch
		self._cache_file = ".cache_" + self._arch + "_" + str(self._batch_size)  # here is: ".cache_resnet18_128"
		...
		self._iter_dur = []		# 每一 iter 用时
		self._mem_util = []		# 每一 iter 占用存储空间
		...
		with open(self._cache_file) as f:
			...
			for key, val in data.items():
				if key == 'avg_iter_dur':
					...
					self._chk_freq = val	# checkpoint频率
				elif key == 'self._chk_fn':	
					self.populate_chk_fn(val)	# self._chk_fn: save使用的函数，self._cf_manager的save()或sava_cpu()
	...
	def cache_params(self):
		...
		with open(self._cache_file, 'w') as f:
			json.dump(data, f)		# 将一些数据写入 self._cache_file文件
			os.fsync(f.fileno())
	...
	def _complete_profile(self, dev):   #profiling： 获取一些基本参数
		self._avg_iter_dur = mean(self._iter_dur)    #iteration平均用时
    ...
    # Profile only the snapshot phase - CPU based
		...
		if t_ct <= t_cp:	# 确定使用 process 还是 thread
			t_c = t_ct		
			self._use_thread = True
		else:
			t_c = t_cp
		...
		overhead = t_c		# CPU-based copy: save time
		#overhead = max(0, t_c - t_w)

		# If mem available, and low overhead, 
		# do synchronous GPU snapshot
		if self._chk_size < self._avg_free_mem:		# _avg_free_mem: 空闲内存
			...
			if t_fp <= t_ft:
					t_f = t_fp		# GPU-based snapshot: save总耗时
					t_g = t_gp		# GPU in-momory copy time
					self._use_thread = False
			else:
					t_f = t_ft
					t_g = t_gt
			...
			t_s = t_f - t_g		# t_s: time to write storage
		...
		self._chk_freq = max(math.ceil((t_f - overhead)/t_i), 1)	# ceil: 向上取整
